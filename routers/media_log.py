...
def list_logs_by_media(media_id: int, db: Session = Depends(get_db)):
    """Lista todos os logs de uma mídia específica."""
    return service.find_by_media(db, media_id)


@router.get("/{log_id}", response_model=MediaLogResponse)
def get_log(log_id: int, db: Session = Depends(get_db)):
    """Busca um log pelo ID."""
    return service.find_by_id(db, log_id)
...
def update_log(log_id: int, data: MediaLogUpdate, db: Session = Depends(get_db)):
    """Atualiza um log existente."""
    return service.update(db, log_id, data)


@router.delete("/{log_id}", status_code=204)
def delete_log(log_id: int, db: Session = Depends(get_db)):
    """Remove um log."""
    service.delete(db, log_id)
